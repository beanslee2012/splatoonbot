...
base_path='E:/game/bot/inkbot/src/'
...
stages = on_command("图", aliases={'下图','下下图' },priority=5)
...
async def stage_handle(bot: Bot, event: Event,state: T_State):
    ...
    draw.text((30,290), GameModeL, (255, 255, 255), font=font)
...
async def coop_handle(bot: Bot, event: Event,state: T_State):
    ...
    base_img = PIL.Image.open(img_path+ 'misc/bgx.jpg')
    radii=20
    base_sep=8
    scale=1.5
    base_xx=20
    base_yy=60
    draw = PIL.ImageDraw.Draw(base_img)
    font = PIL.ImageFont.truetype(img_path+"misc/font/msyh.ttc", 20) 
    ...
    for i in  json.loads(json.dumps(salmon_file))['details']:
        ...
        #print (i['start_time'],i['end_time'],stage[i['stage']['name']]['name'][3:])
